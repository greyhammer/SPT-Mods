...
class SWAG {
        ...
        staticRouterModService.registerStaticRouter(`${modName}/client/raid/configuration`, [
            {
                ...
                action: (url, info, sessionID, output) => {
                    try {
                        ...
                        const aki_bots = configServer.getConfig(ConfigTypes_1.ConfigTypes.BOT);
    ...
    postDBLoad(container) {
        ...
        if (!config_json_1.default?.UseDefaultSpawns?.Waves || !config_json_1.default?.UseDefaultSpawns?.Bosses || !config_json_1.default?.UseDefaultSpawns?.TriggeredWaves) {
        ...
        Object.keys(locations).filter((name) => ClassDef.validMaps.includes(name)).forEach((globalmap) => {
            for (let pattern in customPatterns) {
                ...
                if (mapName === globalmap || mapName === "all") {
                    ...
                    SWAG.randomWaveTimer.time_min = config_json_1.default.GlobalSCAVRandomWaveTimer.WaveTimerMinSec;
                    SWAG.randomWaveTimer.time_max = config_json_1.default.GlobalSCAVRandomWaveTimer.WaveTimerMaxSec;
    ...
    static SetUpStaticBosses(StaticBossGroups, globalmap, AlreadySpawnedBossGroups) {
        ...
        for (let i = 0; i < randomizedBossGroups.length; i++) {
            ...
            let boss_name = ClassDef_1.reverseBossNames[boss.BossName] ? ClassDef_1.reverseBossNames[boss.BossName] : boss.BossName;
            if (actual_boss_name.startsWith("boss") || actual_boss_name.startsWith("useccommander")) {
                let spawnChance = boss.BossChance ? boss.BossChance : bossConfig_json_1.default.BossSpawns[ClassDef_1.reverseMapNames[globalmap]][boss_name].chance;
                if (spawnChance != 0) {
    ...
    static SpawnBosses(boss, globalmap, AlreadySpawnedBossGroups) {
        ...
        if (bossConfig_json_1.default.TotalBossesPerMap[ClassDef_1.reverseMapNames[globalmap]] === 0) {
            config_json_1.default.DebugOutput && logger.info("SWAG: TotalBosses set to 0 for this map, skipping boss spawn");
            return;
        }
        else if (BossWaveSpawnedOnceAlready && (boss_name.startsWith("boss") || boss_name.startsWith("useccommander"))) {
            boss_name = ClassDef_1.reverseBossNames[boss.BossName] ? ClassDef_1.reverseBossNames[boss.BossName] : boss.BossName;
            let spawnChance = boss.BossChance ? boss.BossChance : bossConfig_json_1.default.BossSpawns[ClassDef_1.reverseMapNames[globalmap]][boss_name].chance;
            // if spawn chance is 100 lets ignore the boss limits
            if (bossConfig_json_1.default.TotalBossesPerMap[ClassDef_1.reverseMapNames[globalmap]] === -1 || spawnChance == 100) {
                // do nothing
            }
            else if (SWAG.bossCount.count >= bossConfig_json_1.default.TotalBossesPerMap[ClassDef_1.reverseMapNames[globalmap]]) {
                config_json_1.default.DebugOutput && logger.info("SWAG: Skipping boss spawn as total boss count has been met already");
                return;
            }
        }
    ...
    static ConfigureBotWave(group, bot, globalmap, zone) {
        ...
        const wave = {
            ...
            slots_max: Math.floor(botCount *
                ClassDef_1.aiAmountProper[config_json_1.default.SWAG_ONLY_aiAmount ? config_json_1.default.SWAG_ONLY_aiAmount.toLowerCase() : "asonline"]),
        ...
        if (isRandom) {
            ...
            SWAG.randomWaveTimer.time_min += config_json_1.default.GlobalSCAVRandomWaveTimer.WaveTimerMaxSec;
            SWAG.randomWaveTimer.time_max += config_json_1.default.GlobalSCAVRandomWaveTimer.WaveTimerMaxSec;
        }
    ...
    static ConfigureBossWave(boss, globalmap) {
        ...
        boss?.Supports?.forEach((escort) => {
            escort.BossEscortDifficult = [ClassDef_1.diffProper[config_json_1.default.SWAG_ONLY_aiDifficulty.toLowerCase()]];
        ...
        switch (boss.BossName) {
            ...
            case 'sectantpriest':
                ...
                break;
            case 'pmcbot':
                spawnChance = boss.BossChance ? boss.BossChance : config_json_1.default.spawnChances.raiders[ClassDef_1.reverseMapNames[globalmap]];
                break;
            case 'exusec':
                spawnChance = boss.BossChance ? boss.BossChance : config_json_1.default.spawnChances.rogues[ClassDef_1.reverseMapNames[globalmap]];
                break;
            case 'bloodhound':
                spawnChance = boss.BossChance ? boss.BossChance : config_json_1.default.spawnChances.bloodhounds[ClassDef_1.reverseMapNames[globalmap]];
                break;
            case 'crazyscavs':
                spawnChance = boss.BossChance ? boss.BossChance : config_json_1.default.spawnChances.crazyscavs[ClassDef_1.reverseMapNames[globalmap]];
                break;
            case 'sptbear':
            ...
            case 'sptusec':
                ...
                break;
            case 'marksman':
                spawnChance = boss.BossChance ? boss.BossChance : config_json_1.default.spawnChances.snipers[ClassDef_1.reverseMapNames[globalmap]];
                break;
            case 'assault':
                spawnChance = boss.BossChance ? boss.BossChance : 100;
                break;
            default:
                spawnChance = boss.BossChance ? boss.BossChance : bossConfig_json_1.default.BossSpawns[ClassDef_1.reverseMapNames[globalmap]][bossName].chance;
                spawnTime = boss.Time ? boss.Time : bossConfig_json_1.default.BossSpawns[ClassDef_1.reverseMapNames[globalmap]][bossName].time;
                spawnZones = boss.BossZone ? boss.BossZone : bossConfig_json_1.default.BossSpawns[ClassDef_1.reverseMapNames[globalmap]][bossName].chance;
                break;
        }
    ...
    static disableSpawnSystems() {
        ...
        for (map in locations) {
            ...
            config_json_1.default.DebugOutput && logger.info("SWAG: disabled NewSpawnSystem to clear default spawns (SPT 3.6.x)");
        }
    }
